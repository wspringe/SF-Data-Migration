...
GO
CREATE PROCEDURE [dbo].[Insert_SolarAddendumDetail] (
  @objectName VARCHAR(50)

  /*
    This stored procedure is used for inserting and upserting data for the Solar_Addendum_Detail__cobject.

    Circular definition fields: None.

    Need:

    Cross-Reference: Owner, Solar_Addendum__c
*/
)
AS
  ...
  EXEC Create_Cross_Reference_Table 'Solar_Addendum__c', 'Name'
  ...
  RAISERROR('Replacing Owner with User IDs from target org...', 0, 1) WITH NOWAIT
  SET @SQL = 'update ' + @stagingTable +
  ' set Solar_Addendum__c = x.TargetID
  FROM Solar_Addendum__cXRef x 
  WHERE x.SourceID = ' + @stagingTable + '.Solar_Addendum__c'
  EXEC sp_executeSQL @SQL
