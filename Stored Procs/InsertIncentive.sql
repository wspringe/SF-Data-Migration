...
GO
ALTER PROCEDURE [dbo].[Insert_Incentive] (
...
AS
  ...
  EXEC Create_Id_Based_Cross_Reference_Table 'Incentive_Master__c', @targetLinkedServerName, @sourceLinkedServerName
  ...
  EXEC Replace_NewIds_With_OldIds @stagingTable, 'Incentive_Master__cXref', 'Incentive_Master__c'
  ...
  DECLARE @i INT = 1
  DECLARE @count INT = 0
  SELECT @maxRows = COUNT(Id) FROM Incentive__c_Stage --don't forget to change this
  WHILE @i < @maxRows
  BEGIN
    SET @SQL = 'SELECT *
