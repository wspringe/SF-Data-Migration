...
GO
CREATE PROCEDURE [dbo].[User_FollowUp] (
...
AS
  ...
  SET @SQL = 'EXEC SF_Replicate ''' + @targetLinkedServerName + ''', ''' + @objectName + ''', ''pkchunk''' +
  ...
  RAISERROR ('Creating %s table', 0, 1, @updateTable) WITH NOWAIT
  SET @SQL = 'EXEC SF_Replicate ''' + @sourceLinkedServerName + ''', ''' + @objectName + ''', ''pkchunk'''
  EXEC sp_executesql @SQL
  ...
  EXEC Replace_NewIds_With_OldIds @stagingTable, 'UserXRef', 'DelegatedApproverId' 

  SET @SQL = 'UPDATE ' + @updateTable +
            char(10) + 'SET ContactId  = ' + @stagingTable + '.ContactId' +
            char(10) + 'FROM ' + @updateTable + 
            char(10) + 'JOIN ' + @stagingTable + 
            char(10) + 'ON ' + @updateTable + '.Old_SF_ID__c = ' + @stagingTable + '.Id'
  EXEC sp_executesql @SQL
  SET @SQL = 'UPDATE ' + @updateTable +
            char(10) + 'SET DelegatedApproverId  = ' + @stagingTable + '.DelegatedApproverId' +
            char(10) + 'FROM ' + @updateTable + 
            char(10) + 'JOIN ' + @stagingTable + 
            char(10) + 'ON ' + @updateTable + '.Old_SF_ID__c = ' + @stagingTable + '.Id'
  EXEC sp_executesql @SQL
