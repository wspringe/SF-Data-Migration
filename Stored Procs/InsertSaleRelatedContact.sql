...
GO
ALTER PROCEDURE [dbo].[Insert_SaleRelatedContact] (
...
AS
  ...
  EXEC Replace_NewIds_With_OldIds @stagingTable, 'UserXref', 'Designer_User__c'
  EXEC Replace_NewIds_With_OldIds @stagingTable, 'AccountXref', 'Company_Name__c'
  ...
  DECLARE @i INT = 1
  DECLARE @count INT = 0
  SELECT @maxRows = COUNT(Id) FROM Sale_Related_Contact__c_Stage --don't forget to change this
  WHILE @i < @maxRows
  BEGIN
    SET @SQL = 'SELECT *
