...
GO
ALTER PROCEDURE [dbo].[Insert_Addendum] (
...
AS
  ...
  DECLARE @i INT = 1
  DECLARE @count INT = 0
  SELECT @maxRows = COUNT(Id) FROM Addendum__c_Stage --don't forget to change this
  WHILE @i < @maxRows
  BEGIN
    SET @SQL = 'SELECT *
    ...
    IF @i > @maxRows
      SET @i = @maxRows
    SET @SQL = @SQL + CAST(@i AS NVARCHAR(10)) + ' ORDER BY Lot__c'
    SET @count = @count + 1
