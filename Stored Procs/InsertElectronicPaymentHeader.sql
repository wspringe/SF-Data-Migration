...
GO
CREATE PROCEDURE [dbo].[Insert_ElectronicPaymentHeader] (
...
AS
  ...
  EXEC Create_Id_Based_Cross_Reference_Table 'Electronic_Payment_Queue__c', @targetLinkedServerName, @sourceLinkedServerName


  -- Update stage table with new Ids for Region lookup
  RAISERROR('Replacing Ids from target org...', 0, 1) WITH NOWAIT
  EXEC Replace_NewIds_With_OldIds @stagingTable, 'UserXref', 'Approve_Decline_User__c'
  EXEC Replace_NewIds_With_OldIds @stagingTable, 'Electronic_Payment_Queue__cXref', 'Electronic_Payment_Queue__c'
