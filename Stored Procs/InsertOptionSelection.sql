...
GO
CREATE PROCEDURE [dbo].[Insert_OptionSelection] (
...
AS
  ...
  EXEC Create_Id_Based_Cross_Reference_Table 'Option__c', @targetLinkedServerName, @sourceLinkedServerName


  -- Update stage table with new Ids for Region lookup
  RAISERROR('Replacing Ids from target org...', 0, 1) WITH NOWAIT
  EXEC Replace_NewIds_With_OldIds @stagingTable, 'Option__cXref', 'Option__c'
  EXEC Replace_NewIds_With_OldIds @stagingTable, 'Addendum__cXref', 'Addendum__c'
  EXEC Replace_NewIds_With_OldIds @stagingTable, 'Addendum__cXref', 'Parent_Addendum__c'
  ...
  DECLARE @i INT = 1
  DECLARE @count INT = 0
  SELECT @maxRows = COUNT(Id) FROM Option_Selection__c_Stage --don't forget to change this
  WHILE @i < @maxRows
  BEGIN
    SET @SQL = 'SELECT *
    ...
    IF @i > @maxRows
      SET @i = @maxRows
    SET @SQL = @SQL + CAST(@i AS NVARCHAR(10)) + ' ORDER BY Addendum__c'
    SET @count = @count + 1
