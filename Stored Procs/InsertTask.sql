...
AS
  ...
  EXEC Create_Id_Based_Cross_Reference_Table 'School__c', @targetLinkedServerName, @sourceLinkedServerName
  EXEC Create_Id_Based_Cross_Reference_Table 'Addendum__c', @targetLinkedServerName, @sourceLinkedServerName
  EXEC Create_Id_Based_Cross_Reference_Table_Standard_Objects 'Case', @targetLinkedServerName, @sourceLinkedServerName
  EXEC Create_Id_Based_Cross_Reference_Table 'Custom_Option_Tracker__c', @targetLinkedServerName, @sourceLinkedServerName
  EXEC Create_Id_Based_Cross_Reference_Table 'Lot_Start__c', @targetLinkedServerName, @sourceLinkedServerName
  EXEC Create_Id_Based_Cross_Reference_Table 'Budget_Maintenance_Tracker__c', @targetLinkedServerName, @sourceLinkedServerName
  EXEC Create_Id_Based_Cross_Reference_Table 'Sale_Related_Contact__c', @targetLinkedServerName, @sourceLinkedServerName
  EXEC Create_Id_Based_Cross_Reference_Table 'Realtor_Registration__c', @targetLinkedServerName, @sourceLinkedServerName
  ...
  EXEC Replace_NewIds_With_OldIds @stagingTable, 'Marketing_Area__CXRef', 'WhatId'
  EXEC Replace_NewIds_With_OldIds @stagingTable, 'OpportunityXRef', 'WhatId'
  EXEC Replace_NewIds_With_OldIds @stagingTable, 'Sale__cXRef', 'WhatId'
  EXEC Replace_NewIds_With_OldIds @stagingTable, 'School__cXRef', 'WhatId'
  EXEC Replace_NewIds_With_OldIds @stagingTable, 'Addendum__cXRef', 'WhatId'
  EXEC Replace_NewIds_With_OldIds @stagingTable, 'CaseXRef', 'WhatId'
  EXEC Replace_NewIds_With_OldIds @stagingTable, 'Custom_Option_Tracker__cXRef', 'WhatId'
  EXEC Replace_NewIds_With_OldIds @stagingTable, 'Lot_Start__cXRef', 'WhatId'
  EXEC Replace_NewIds_With_OldIds @stagingTable, 'Budget_Maintenance_Tracker__cXRef', 'WhatId'
  EXEC Replace_NewIds_With_OldIds @stagingTable, 'Sale_Related_Contact__cXRef', 'WhatId'
  EXEC Replace_NewIds_With_OldIds @stagingTable, 'Realtor_Registration__cXRef', 'WhatId'
  ...
  DECLARE @i INT = 1
  DECLARE @count INT = 0
  SELECT @maxRows = COUNT(Id) FROM Task_Stage --don't forget to change this
  WHILE @i < @maxRows
  BEGIN
    SET @SQL = 'SELECT *
    ...
    IF @i > @maxRows
      SET @i = @maxRows
    SET @SQL = @SQL + CAST(@i AS NVARCHAR(10)) + ' ORDER BY WhatId'
    SET @count = @count + 1
  ...
  WHILE @i < @count
  BEGIN
      SET @SQL = 'EXEC SF_Tableloader ''Upsert'', ''' + @targetLinkedServerName + ''', ''' + @stagingTable + '_Split' + CAST(@i AS NVARCHAR(10)) + ''', ''Old_SF_ID__C'''
      SET @i = @i + 1
      EXEC sp_executeSQL @SQL
  END
